...
class Car:
    ...
    def __init__(self, ID, initialPos, lane, vMax, tolerance, dMin) -> None:
        # id of the car
        self.ID = ID
        ...
        for k in range(10):
            ...
            for i in range(l):
                for j in range(cars[i]):
                    mat[i][j] = Car(ID, j*hBar[i], i, 40, 500, random.gauss(10, 2))
                    ID += 1
            ...
            for a in range(l):
                for b in range(n):
                    if (mat[a][b] is None):
                        ...
                        print(str(mat[a][b].ID) + "-" + str(mat[a][b].getPosition(0)) + "-" + str(mat[a][b].dMin), end=" ")
                print("\n")
            # list of times
            tList = [0]
            # the time
