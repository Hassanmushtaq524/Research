...
def main():
    ...
    fileName =  os.getcwd() + "\\multilane\\trafficLight\\Flow\\imbalancedLoad\\differentSwitchingTime\\" + str(n) + ".txt"
    print(fileName)
    try:
        ...
        for num in range(5, n//2 + 1, 5):
            ...
            while switchingT <= 150:
                # print("switchingt: " + str(switchingT))
                # SIMULATION
                ...
                count = 0
                # when to end
                endTime = 5*switchingT + 15*switchingT
                # start the time
                while t <= endTime:
                    t += 0.01
                    timeStep += 1
                    ...
                    if (timeStep == nextSwitch):
                        ...
                        if (signal):
                            nextSwitch = timeStep + 3*(switchingT/0.01)
                        else:
                    ...
                    if ((timeStep >= (endTime-4*switchingT)/0.01) and (timeStep <= (endTime/0.01))):
